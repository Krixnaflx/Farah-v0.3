// © Author: Krixnaflx// https://discord.gg/BWRS8VHFYr

const { SlashCommandBuilder, PermissionFlagsBits, AttachmentBuilder } = require('discord.js');const {ContainerBuilder, TextDisplayBuilder, SeparatorBuilder,SeparatorSpacingSize, MessageFlags} = require('discord.js');const { LoggingConfig, GuildConfig } = require('../../data/models');

const LINK_REGEX = /https?://[^\s]+|www.[^\s]+|discord.gg/[^\s]+/i;const TWO_WEEKS = 14 * 24 * 60 * 60 * 1000;

function reply(ctx, text, ephemeral = false) {const container = new ContainerBuilder().setAccentColor(0x2B2D31).addTextDisplayComponents(new TextDisplayBuilder().setContent(text));const opts = { components: [container], flags: MessageFlags.IsComponentsV2 };if (ephemeral) opts.ephemeral = true;return ctx.reply(opts);}

function buildFilter(type, targetUserId) {const twoWeeksAgo = Date.now() - TWO_WEEKS;const base = msg => msg.createdTimestamp > twoWeeksAgo && !msg.pinned;
...
async function bulkDeleteMessages(channel, amount, filterFn) {let totalDeleted = 0;let lastMessageId = null;const userCounts = new Map();const deletedLogs = [];let remaining = amount;
...
async function sendPurgeLog(logChannelId, guild, channel, totalDeleted, filterLabel, member, deletedLogs) {const logChannel = await guild.channels.fetch(logChannelId).catch(() => null);if (!logChannel) return;
...
module.exports = {data: new SlashCommandBuilder().setName('purge').setDescription('Bulk delete messages from the channel').setDefaultMemberPermissions(PermissionFlagsBits.ManageMessages).addIntegerOption(opt =>opt.setName('amount').setDescription('Number of messages to delete (1-500)').setRequired(true).setMinValue(1).setMaxValue(500)).addStringOption(opt =>opt.setName('filter').setDescription('Filter messages to delete').addChoices({ name: 'Bots - Messages from bots', value: 'bots' },{ name: 'Humans - Messages from humans', value: 'humans' },{ name: 'Attachments - Messages with files/images', value: 'attachments' },{ name: 'Links - Messages containing links', value: 'links' },{ name: 'Embeds - Messages with embeds', value: 'embeds' },)).addUserOption(opt =>opt.setName('user').setDescription('Only delete messages from this user')).addStringOption(opt =>opt.setName('contains').setDescription('Only delete messages containing this text')),
